...
def parse_response(text: str):
    ...
    返回:
      label_norm: "APT" / "Benign" / None
      conf_norm:  "high" / "medium" / "low" / None
    """
    text = text.strip()

    # 有些模型会在 JSON 前后加废话，这里截取第一个 '{' 到最后一个 '}' 之间
...
def main():
    ...
    for i, (idx, row) in enumerate(df_sample.iterrows(), start=1):
        ...
        try:
            ...
            if label_raw is None:
                ...
                print(content)
                # 简单兜底：解析失败时默认判为 Benign
                pred_label = "Benign"
            else:
                pred_label = label_raw

                # === 这里是“门控”逻辑核心 ===
                # 只信任 high-confidence 的 APT
                if pred_label == "APT":
                    if conf != "high":
                        # 降级为 Benign
                        downgraded_count += 1
    ...
    print("\n=== LLM Baseline Evaluation on Sampled Test Set (with confidence gating) ===")
    print(f"APT predictions downgraded to Benign due to low/unknown confidence: {downgraded_count}")
    # 固定标签顺序，方便阅读
    labels = ["APT", "Benign"]
